        ...
        if img.ndim == 3:
            if isinstance(self.size, int):
                if img.shape[0] > img.shape[1]:
                    return np.array(Image.fromarray(obj=img).resize(size=(int(self.size * img.shape[0] / img.shape[1]), self.size)))
                else:
                    return np.array(Image.fromarray(obj=img).resize(size=(self.size, int(self.size * img.shape[1] / img.shape[0]))))
            elif isinstance(self.size, float):
                return np.array(Image.fromarray(obj=img).resize(size=(int(self.size*img.shape[0]), int(self.size*img.shape[0]))))
            elif (isinstance(self.size, collections.Iterable) and len(self.size) == 2):
                return np.array(Image.fromarray(obj=img).resize(size=self.size))
            else:
                print('shit1')
            #return misc.imresize(img, self.size, self.interpolation)
        elif img.ndim == 2:
            if isinstance(self.size, int):
                return np.array(Image.fromarray(obj=img, mode='F').resize(size=(self.size, self.size)))
            elif isinstance(self.size, float):
                return np.array(Image.fromarray(obj=img, mode='F').resize(size=(int(self.size*img.shape[0]), int(self.size*img.shape[0]))))
            elif (isinstance(self.size, collections.Iterable) and len(self.size) == 2):
                return np.array(Image.fromarray(obj=img, mode='F').resize(size=self.size))
            else:
                print('shit2')
